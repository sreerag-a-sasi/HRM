  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (file) {
        let mime_type = file.split(';')[0].split(':')[1].split('/')[1];
        console.log("mime_type:", mime_type);
        if (['mp4', 'png', 'jpg', 'jpeg', 'pdf'].includes(mime_type)) {
          console.log("Allowed file type");
          ...
          fs.mkdir(upload_path, { recursive: true }, (err) => {
            if (err) {
              ...
              let final_upload_path = `${upload_path}/${file_name}`;
              console.log("upload_path:", final_upload_path);
              fs.writeFile(final_upload_path, base64, { encoding: 'base64' }, function (err) {
                if (err) {
                  console.log("err:", err);
                  reject(err.message ? err.message : err);
                } else {
